...
plot(st_geometry(hunan_m), border="lightgrey")
plot(wm_d62, coords_m, add=TRUE)
plot(k1_m, coords_m, add=TRUE, col="red", length=0.08)
```
...
```{r}
#| label: course-six-neighbours-structure
#| eval: false
str(knn6)
```

```{r}
#| label: course-six-neighbours-plot
#| eval: false
plot(hunan$geometry, border="lightgrey")
...
```{r}
#| label: course-neighbour-gdppc-repeat
#| eval: false
nb1 <- wm_q[[1]]
nb1 <- hunan$GDPPC[nb1]
nb1
...
```{r}
#| label: course-self-neighbours-first
#| eval: false
wm_qs[[1]]
```

```{r}
#| label: course-self-neighbours-listw
#| eval: false
