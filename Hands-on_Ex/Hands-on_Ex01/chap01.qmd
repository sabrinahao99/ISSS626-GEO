...
This exercise follows the Chapter 1 workflow and rebuilds the introductory geospatial data science operations with local data. The main aim is to practise importing spatial and aspatial data, checking coordinate reference systems, converting tabular coordinates into `sf` features, and carrying out basic geometric operations.

::: {.callout-note}
## Local Implementation Note

The original teaching page reads several files from a `chap01/data` folder. In this website, the data has been downloaded into `Hands-on_Ex/Hands-on_Ex01/data`, so the code below uses local relative paths such as `data/geospatial` and `data/aspatial/listings.csv`.
:::
...
The overlay confirms that both layers now share the same projected coordinate space. This matters because the later buffer, area, and density calculations depend on metre-based distances.
:::
...
The cycling path layer was downloaded from LTA DataMall's current geospatial release, so the total buffered area can differ from the course page if the network has been updated.
:::
